...
class PromptTokenCounter:
    ...
    def count_prompt_parts(
        ...
        *,
        ...
        """Return token counts for each prompt section and the compiled total."""
        tokenizer = self._get_tokenizer(hf_id)
        if tokenizer is None:
            return {
                "prepend": _estimate_tokens(prepend_text),
                "main": _estimate_tokens(main_text),
                "append": _estimate_tokens(append_text),
                "total": _estimate_tokens(compiled_prompt),
                "method": "heuristic",
            }

        return {
            "prepend": self._count_text(tokenizer, prepend_text),
            "main": self._count_text(tokenizer, main_text),
            "append": self._count_text(tokenizer, append_text),
            "total": self._count_text(tokenizer, compiled_prompt, apply_chat_template=True),
            "method": "tokenizer",
        }

    def count_prompt_sections(
        self,
        *,
        hf_id: str | None,
        subject_text: str,
        ...
        """Return token counts for the five-section prompt composer schema."""
        tokenizer = self._get_tokenizer(hf_id)
        if tokenizer is None:
            counts = {
                "subject": _estimate_tokens(subject_text),
                "setting": _estimate_tokens(setting_text),
                "details": _estimate_tokens(details_text),
                "lighting": _estimate_tokens(lighting_text),
                "atmosphere": _estimate_tokens(atmosphere_text),
                "total": _estimate_tokens(compiled_prompt),
                "method": "heuristic",
            }
        else:
            counts = {
                "subject": self._count_text(tokenizer, subject_text),
                "setting": self._count_text(tokenizer, setting_text),
                "details": self._count_text(tokenizer, details_text),
                "lighting": self._count_text(tokenizer, lighting_text),
                "atmosphere": self._count_text(tokenizer, atmosphere_text),
                "total": self._count_text(tokenizer, compiled_prompt, apply_chat_template=True),
                "method": "tokenizer",
            }

        # Backward-compat aliases expected by existing clients.
